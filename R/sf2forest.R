...
#' @param year year of samplig , NA as deault for ES
...
forestplotlist <- function(id, year = NA, country, version = NA, tree, regen, shrub,herbs, filterNA, filterDead, minDBH,filterminDBH, setDefaults, .verbose = TRUE){
  ...
  if (nrow(treeData)>0){
    ...
    if(.verbose) cat("Processing tree data...\n")
  ...
  if (nrow(regenData)>0) {
    ...
    if(.verbose) cat("Processing regendata...\n")
  ...
  if (nrow(shrubData)>0) {
    ...
    if(.verbose) cat("Processing shrub data..\n")
...
sf2forest <- function(input_df,country, filterNA, filterDead, minDBH,filterminDBH, setDefaults, .verbose = TRUE) {
  ...
  if (is.null(input_df)) {
    if (interactive()) {
      cli::cli_inform(c(
        "You have not specified an input"
      ))
    }
  }
  ...
  purrr::pmap(
    ...
    .f = \(id,
      ...
      if (is.na(version)){
        forest<- forestplotlist(id,year,country, version = NA,tree, regen, shrub, herbs, filterNA, filterDead,  minDBH, filterminDBH, setDefaults, .verbose)
      }

      if (version %in% c("ifn2","ifn3","ifn4")){

        forest<- forestplotlist(id,year,country, version ,tree, regen, shrub, herbs, filterNA, filterDead,  minDBH, filterminDBH, setDefaults,.verbose)
      }
